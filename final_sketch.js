...
function draw() {
  ...
  if (clusterX < 0 || clusterX > width) {
    flySpeedX = -flySpeedX;
  }
  if (clusterY < 0 || clusterY > height) {
    flySpeedY = -flySpeedY;
  }
...
function drawHours(myHour){
  ...
  for (let j=0; j<myHour;j++) {
    let theta = map(j,0,60,-HALF_PI,PI+HALF_PI)
    let radius = 100;
    //calculating points (from the center)
    let xpos = width/3 + radius*cos(theta)
    let ypos = height/1.3 + radius*sin(theta)
    fill('green')
    ellipse(xpos,ypos,15,7,15,7)
  }
  for(let j=0; j < myHour; j++){
...
function drawSeconds(mySecond) {
  ...
  for (let k = 0; k < mySecond; k++) {
    ...
    const angle = random(0, clusterRadius);
